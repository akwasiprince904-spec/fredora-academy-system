...
router.get('/stats/overview', async (req, res) => {
  try {
    ...
    // Use Postgres-safe CASE with single quotes and cast counts to int
    const statsPromise = db('students')
      .select(
        db.raw("COUNT(*)::int as total_students"),
        db.raw("SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END)::int as active_students"),
        db.raw("SUM(CASE WHEN status = 'inactive' THEN 1 ELSE 0 END)::int as inactive_students"),
        db.raw("SUM(CASE WHEN status = 'graduated' THEN 1 ELSE 0 END)::int as graduated_students"),
        db.raw("SUM(CASE WHEN gender = 'male' THEN 1 ELSE 0 END)::int as male_students"),
        db.raw("SUM(CASE WHEN gender = 'female' THEN 1 ELSE 0 END)::int as female_students"),
        // Compute total fee balance from payments as a fallback if students table has no fee_balance column
        db.raw('(SELECT COALESCE(SUM(amount_due - amount_paid), 0)::numeric FROM payments) as total_fee_balance')
      )
      .first();
    ...
    const classDistributionPromise = db('students')
      .select(
        ...
        db.raw('COUNT(students.id)::int as student_count')
      )
    ...
    const [overview, distribution] = stats && classDistribution ? [stats, classDistribution] : [{}, []];
    ...
    console.error('Get student overview stats error:', error);
    return res.status(500).json({ success: false, message: 'Error fetching student overview stats' });
  }
});
